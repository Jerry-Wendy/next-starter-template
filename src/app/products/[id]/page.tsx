...
import { addPricingTier, deletePricingTier, updateCatalogSettings, updateProductColors, uploadProductImage, uploadBusinessImage } from "./actions";
    ...
    <main className="min-h-screen bg-slate-50 p-8">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Internal supplier/reference image. Never shown in the Customer Catalog.
        </p>
        ...
        {product.business_image_url ? (
          <div className="mt-4">
            <img
              ...
              alt={`${product.name} internal`}
              className="max-h-64 rounded-lg border border-slate-200 object-contain"
            />
          </div>
        ) : (
          <p className="mt-3 text-sm text-slate-500">
            No internal image uploaded yet.
          </p>
        )}

        <form
          action={uploadBusinessImage}
          className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center"
        >
          <input type="hidden" name="product_id" value={product.id} />
